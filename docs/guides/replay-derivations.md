...
Use this guide to measure rebuilding on an isolated database copy. A derivation
is saved output produced from recorded inputs; the replay initializes those
outputs through the ordinary project and link workers. It is not a production
restore or a release acceptance run.

Read [current status](../status.md) and its linked handoff for the reviewed
runtime. Set `H16_SOURCE` to that frozen source root, `H16_PYTHON` to its Python
interpreter, `H16_RECEIPT` to the reviewed source receipt hash, and
`REPLAY_DRIVER` to the reviewed `journal/tools/runtime/replay_derivations.py` script. H16 is
the original work-package ID for release evidence. Commands below also require
the native-library environment used by that runtime.

`journal/tools/runtime/replay_derivations.py` measures initialization through the real
