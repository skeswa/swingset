...
scoring.dance provides event results and round sheets, sometimes including WSDC dancer numbers. This guide owns its page and cache behavior.

[All sources](README.md) · [Shared fetching rules](../fetching.md)

## 1. Status

Verified 2026-09-08 (`journal/evidence/collection/source-survey-2026-09-08/sd_*.hdr`).
...
- So: send `If-Modified-Since` anyway (free 304s while the edge is
  ...
  hash of the page kind's `extract` output (`docs/reference/parsing.md`), so it
...
From `docs/reference/parsing.md` and `journal/investigations/undated/initial-source-survey.md`:
