...
The registry provides WSDC dancer numbers, points, and placements. It is used to check identities and find event history.

[All sources](README.md) · [Shared fetching rules](../fetching.md)

## 1. Status

General facts verified 2026-09-04 in `journal/investigations/undated/initial-source-survey.md`; found and missing
...
`wsdc_registry.dancer`: JSON shape in `journal/investigations/undated/initial-source-survey.md`. Quirks:
...
`journal/investigations/2026/prior-art-registry-analyses.md`. The lookup's JSON has changed
...
From `journal/archive/initial-open-questions.md`: meaning of `adv_sliding` and `as_sliding`, whether merged numbers are
