...
The Wayback Machine provides saved copies of older source pages. This guide owns archive requests and capture handling; the backfill reference owns which historical evidence to select.

[All sources](README.md) · [Shared fetching rules](../fetching.md)

## 1. Status

Verified 2026-09-11 (`journal/evidence/collection/wayback-2026-09-11/`). The Internet
...
`journal/investigations/2026/wayback-coverage-2026-09-11.md`.
...
`docs/reference/backfill.md` owns it. Per source, per capture year, one paged
...
2. For each URL, select a capture (`docs/reference/backfill.md`, "Selecting a
...
About 8,000 requests in total for the 2010 start (`docs/reference/backfill.md`,
...
are in `journal/evidence/collection/phase1-2026-09-13/wp11-eepro2019/`;
`journal/tools/collection/accept_wayback_event.py` reproduces the bounded check.
...
`journal/tools/collection/intake_phase1.py` is the reproducible orchestration entry point.
...
```sh
python -m journal.tools.collection.intake_phase1 bootstrap \
  --checkpoint /path/to/read-only-checkpoint --state /path/to/intake
python -m journal.tools.collection.intake_phase1 catalog --state /path/to/intake
python -m journal.tools.collection.intake_phase1 run --state /path/to/intake \
  --max-targets 180 --wall-seconds 3000
python -m journal.tools.collection.intake_phase1 run --state /path/to/intake \
  --retry-failures --max-targets 180 --wall-seconds 3000
python -m journal.tools.collection.intake_phase1 discover --state /path/to/intake --wall-seconds 900
python -m journal.tools.collection.intake_phase1 run --state /path/to/intake --max-targets 180
python -m journal.tools.collection.intake_phase1 project --state /path/to/intake
python -m journal.tools.collection.intake_phase1 review --state /path/to/intake \
  --output journal/evidence/collection/phase1-2026-09-13
```
