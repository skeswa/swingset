...
EEPro provides result indexes and round sheets. This guide explains how to find changed files without repeatedly downloading every sheet.

[All sources](README.md) · [Shared fetching rules](../fetching.md)

## 1. Status

Verified 2026-09-08 (headers in `journal/evidence/collection/source-survey-2026-09-08/eepro_*.hdr`).
...
`eepro.round` (from `docs/reference/parsing.md`, verified 2026-09-09 fixtures):
...
1. One paged CDX query per capture year of `eepro.com/results/*`
   (`docs/reference/backfill.md`); rows land in `archive_captures` and become
   `backfill` watches.
