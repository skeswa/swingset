...
print(json.dumps({'at':value['at'],'unit':value['unit'],'service':value['service'],'materialized':value['materialized'],'attempt_outcomes':value['attempt_outcomes'],'receipt':{k:value['receipt'].get(k) for k in ('status','elapsed_seconds','completed','generation_count','unfinished_by_scope','error_type')}},sort_keys=True))
