...
def assemble(plan_path,plan_sha,output=None):
    ...
    if plan.get('validation_pending') is not False or not plan.get('validation_receipt') or not plan.get('validation_receipt_sha256'):
        raise ValueError('actual successful full validation has not been bound')
    validation=repo/relative(plan['validation_receipt'])
    if plan['overlay'].get(plan['validation_receipt'])!=plan['validation_receipt_sha256']:
        raise ValueError('actual validation receipt is absent from source overlay')
    ...
    if not isinstance(plan.get('required_test_count'),int) or plan['required_test_count']<=0 or tested['tests']['passed']!=plan['required_test_count']:
        raise ValueError('expected full reviewed test result')
