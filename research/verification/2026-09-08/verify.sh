...
( # eepro
  ...
  sleep 6
  et=$(etag_of eepro_round); lm=$(lm_of eepro_round)
  ...
  sleep 6
  et=$(etag_of eepro_index); lm=$(lm_of eepro_index)
...
( # scoring.dance
  ...
  sleep 6
  et=$(etag_of sd_event); lm=$(lm_of sd_event)
...
( # dcn
  ...
  sleep 6
  et=$(etag_of dcn_results); lm=$(lm_of dcn_results)
...
( # wdr
  ...
  sleep 6
  et=$(etag_of wdr_awards); lm=$(lm_of wdr_awards)
...
( # wsdc calendar
  ...
  sleep 6
  et=$(etag_of wsdc_events); lm=$(lm_of wsdc_events)
...
wait
echo; echo "### headers of interest"
for f in *.hdr; do echo "--- $f"; grep -iE '^(HTTP/|etag|last-modified|cache-control|content-encoding|content-type|content-length|cf-cache-status|x-cache|server|vary|age|x-amz|via|retry-after|ratelimit)' "$f" | tr -d '\r'; done
