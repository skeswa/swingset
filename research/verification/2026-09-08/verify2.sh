...
get() { local name=$1 url=$2; shift 2
...
( get wsdc_events https://worldsdc.com/events/
  sleep 6; et=$(etag_of wsdc_events); lm=$(lm_of wsdc_events)
  get wsdc_events_cond https://worldsdc.com/events/ ${et:+-H "If-None-Match: $et"} ${lm:+-H "If-Modified-Since: $lm"} ) &
wait
for f in dcn_results dcn_results_cond dcn_robots sd_event sd_event_cond wdr_awards_routeinfo wdr_awards_routeinfo_cond wsdc_events wsdc_events_cond; do echo "--- $f.hdr"; grep -iE '^(HTTP/|etag|last-modified|cache-control|content-encoding|content-length|cf-cache-status|x-cache|age|expires)' "$f.hdr" | tr -d '\r'; done
echo "--- dcn robots"; cat dcn_robots.body
echo "--- dcn body sizes"; wc -c dcn_results.body dcn_results_cond.body; cmp dcn_results.body dcn_results_cond.body && echo IDENTICAL || echo DIFFER
grep -o 'publishCompResults[^,]*' dcn_results.body | head -2
echo "--- sd body"; wc -c sd_event.body sd_event_cond.body; cmp sd_event.body sd_event_cond.body && echo IDENTICAL || echo DIFFER
echo "--- wsdc body"; wc -c wsdc_events.body wsdc_events_cond.body; cmp wsdc_events.body wsdc_events_cond.body && echo IDENTICAL || echo DIFFER
