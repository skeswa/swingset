...
result={'source':str(source),'state':str(state),'read_only':True,'network_requests':0,'worker_executed':False}
...
Path('/var/tmp/swingset-h16-link-selection-profile.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n')
