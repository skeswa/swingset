{ pkgs, source ? ../. }:
let
  sourcePath = if builtins.isAttrs source then source.outPath else builtins.path {
    path = source;
  ...
  sourceIdentity = if builtins.isAttrs source && source ? rev then source.rev
    else "uncommitted:${builtins.baseNameOf sourcePath}";
in
