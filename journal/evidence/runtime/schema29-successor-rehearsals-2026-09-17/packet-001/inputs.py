...
SOURCE_RECEIPT = "44208cb5b38f0cd3f8b6c7c91f1b5d1cd650ef677d52b4bd54c17d8576c485e7"
...
PROTECTED += ('host_request_spacing', 'host_request_spacing_baselines', 'scheduler_event_turns', 'scheduler_event_requests', 'scheduler_capacity_requests', 'history_origin_requests')
HISTORY = ('event_stage_operations', 'event_progress_receipts', 'event_accounting_receipts', 'event_retirement_receipts', 'source_event_retirement_receipts', 'event_timing_history')
...
def prepare(checkpoint: Path, scratch: Path, source: Path, util: Any) -> dict[str, Any]:
    ...
    manifest = verify_checkpoint(checkpoint, maximum_schema_version=29)
    require(
        ...
        "checkpoint is not held acknowledged schema28 predecessor",
    )
    with closing(
    ...
    try:
        if args.phase == "prepare":
            ...
            with open_database(scratch, read_only=True) as inspected:
                require(inspected.schema_version == 29, "scratch phase cannot authorize migration")
            with open_database(scratch) as db:
                require(
                    protected(
