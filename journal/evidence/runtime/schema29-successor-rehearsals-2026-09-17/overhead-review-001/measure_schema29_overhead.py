...
This bounded paired microbenchmark isolates no-op update trigger overhead. It
does not measure parsing, acquisition, durable commit cost, or fleet service.
"""
...
def fixed_cohort(
    ...
    """Pick one existing row from each safe, nonempty trigger target table."""
    foreign_triggers = {
        str(table): str(name)
        for name, table in conn.execute(
            "SELECT name,tbl_name FROM sqlite_master WHERE type='trigger' "
            "AND name NOT LIKE ? ORDER BY tbl_name,name",
            (TRIGGER_PREFIX + "%",),
        )
    }
    table_modes = {
    ...
    for table in sorted(trigger_tables):
        if foreign_triggers.get(table):
            skipped.append(table + ":other_trigger")
            continue
        if table_modes.get(table, False):
            skipped.append(table + ":without_rowid")
            continue
        cols = [str(row[1]) for row in conn.execute(f"PRAGMA table_info({quoted(table)})")]
        if not cols:
            skipped.append(table + ":no_columns")
            continue
        row = conn.execute(f"SELECT rowid FROM {quoted(table)} ORDER BY rowid LIMIT 1").fetchone()
        if row is None:
            skipped.append(table + ":empty")
            continue
        cohort.append(dict(table=table, rowid=int(row[0]), column=cols[0]))
    require(len(cohort) >= MIN_COHORT, f"safe nonempty cohort has fewer than {MIN_COHORT} tables")
    ...
    try:
        ...
        for _ in range(rounds):
            for item in cohort:
                ...
                conn.execute(
                    f"UPDATE {table} SET {column}={column} WHERE rowid=?", (item["rowid"],)
                )
...
def paired_measurement(
    ...
    *,
    ...
    """Compare the same populated database and rows with/without v29 triggers."""
    ...
    before_rows = {
        (item["table"], item["rowid"]): conn.execute(
            f"SELECT * FROM {quoted(item['table'])} WHERE rowid=?", (item["rowid"],)
        ).fetchone()
        for item in cohort
    }
    baseline: list[int] = []
...
def run(
    ...
    report.update(
        ...
        passed=True,
        scope=(
            "paired no-op UPDATEs on one fixed set of nonempty schema29 trigger target rows; "
            "only history_timing_* triggers are removed in baseline savepoints"
        ),
        limitations=[
            "This measures transactional no-op UPDATE and trigger cost on actual nonempty rows in one copied migration specimen; it excludes durable commit/fsync cost, parsing, selection and network service.",
            "Rows with other triggers, empty tables or WITHOUT ROWID tables are excluded from the fixed cohort; the report lists every excluded migration target.",
