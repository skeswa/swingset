...
def guards() -> dict[str, str]:
    ...
    actual = {p.relative_to(SOURCE).as_posix() for p in children if p.is_file() and "__pycache__" not in p.parts and p != SOURCE / "extension-source.json"}
    require(not any(p.is_symlink() for p in children) and actual == set(receipt["files"]), "runtime inventory differs")
    for name, value in receipt["files"].items():
        require(not Path(name).is_absolute() and ".." not in Path(name).parts, "source receipt path escapes")
...
def main() -> None:
    ...
    require(args.output.resolve() != STATE and not args.output.resolve().is_relative_to(SOURCE) and "checkpoints" not in args.output.resolve().parts, "unsafe output location")
    ...
    try:
        with db_module.open_database(STATE, read_only=True, lock=True) as database, control_lock(STATE, timeout=0):
            guards()
            conn = database.connection
            require(database.schema_version == 28 and conn.execute("PRAGMA user_version").fetchone()[0] == 28, "live schema changed")
            require(
            ...
            if args.checkpoint:
                ...
                checkpoint = create_checkpoint(
                    STATE, conn, args.checkpoint, schema_version=28,
                    versions=versions(), input_bundle_hash=BUNDLE,
                )
