...
def main() -> None:
    ...
    require(not args.resume_checkpoint or args.checkpoint is not None, "resume requires checkpoint")
    if args.checkpoint:
        require(args.checkpoint.exists() == args.resume_checkpoint, "checkpoint existence differs from requested mode")
        ...
        with db_module.open_database(STATE, read_only=True, lock=True) as database:
            ...
            if args.checkpoint:
                if args.resume_checkpoint:
                    ...
                    manifest = verify_checkpoint(args.checkpoint, maximum_schema_version=14)
                    require(manifest["input_bundle_hash"] == BUNDLE, "checkpoint bundle differs")
                    checkpoint = Checkpoint(args.checkpoint, sha(args.checkpoint / "checkpoint.json"), manifest["files"])
                else:
                ...
                print(json.dumps(dict(phase="checkpoint_verified", **report["checkpoint"])), flush=True)
