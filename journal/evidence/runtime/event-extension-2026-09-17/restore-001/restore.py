...
def frozen_util(source: Path, receipt: Path, expected: str) -> Any:
    ...
    require(sha(source / name) == (recorded["sha256"] if isinstance(recorded, dict) else recorded), "migration helper differs")
...
class ReadOnlyHeldHub:
    ...
    def held(self) -> None:
        require(sha(self.state / "operator-hold") == self.hold_sha, "restored operator hold changed")

    def head(self) -> str:
        from swingset.backup.checkpoint import _verify_candidate_remote

        self.held()
        require((self.state / "RESTORE_PENDING").is_file(), "restore barrier cleared before remote check")
        value = self.hub.head()
        require(value == self.baseline, f"public baseline differs: expected {self.baseline!r}, actual {value!r}")
...
def exercise_restore(checkpoint: Path, state: Path, manifest: dict[str, Any], hub: Any, clock: Any, *, maximum_schema: int) -> ReadOnlyHeldHub:
    from swingset.backup.checkpoint import restore_from_checkpoint

    require(not state.exists(), "restore target must be new")
    checked = ReadOnlyHeldHub(hub, state, hold_sha=manifest["files"]["operator-hold"]["sha256"], baseline=BASELINE)
    restore_from_checkpoint(checkpoint, state, checked, clock, lock_timeout=0, maximum_schema_version=maximum_schema)
    checked.held()
    require(not (state / "RESTORE_PENDING").exists(), "restore activation did not finish")
    require(len(checked.heads) == 2 and checked.remote is not None, "remote restore checks incomplete")
    return checked
...
class SerialReadTransport(httpx.BaseTransport):
    ...
    def __init__(self, inner: httpx.BaseTransport, user_agent: str, *, monotonic: Callable[[], float] = time.monotonic, sleep: Callable[[float], None] = time.sleep) -> None:
        self.inner, self.user_agent, self.monotonic, self.sleep = inner, user_agent, monotonic, sleep
    ...
    def handle_request(self, request: httpx.Request) -> httpx.Response:
        ...
        try:
            ...
            self.requests.append(dict(method=request.method, host=host, issued_at=datetime.now(UTC).isoformat()))
            response = self.inner.handle_request(request)
            if not isinstance(response.stream, httpx.SyncByteStream):
                raise ValueError("unexpected asynchronous response")
            return httpx.Response(response.status_code, headers=response.headers, extensions=response.extensions, stream=HeldStream(response.stream, finish))
        except BaseException:
            finish()
            raise
...
def remote_reader(repo: str, cache: Path) -> Any:
    ...
    require("huggingface_hub.constants" not in sys.modules, "remote client must initialize with disposable cache controls")
...
def runtime_bound(source: Path) -> None:
    for name, module in tuple(sys.modules.items()):
        ...
        if name == "swingset" or name.startswith("swingset."):
            require(path is not None and Path(path).resolve().is_relative_to(source / "src"), f"runtime import escaped frozen source: {name}")
...
def main() -> None:
    ...
    os.umask(0o077)
    require(len(args.checkpoint_sha256) == 64 and all(c in "0123456789abcdef" for c in args.checkpoint_sha256), "invalid checkpoint digest")
    require(len(args.archive_commit) == 40 and all(c in "0123456789abcdef" for c in args.archive_commit), "invalid archive commit")
    ...
    require(not any(destination.is_relative_to(p) or p.is_relative_to(destination) for p in (PRODUCTION, source, checkpoint)), "destination overlaps protected state or inputs")
    util = frozen_util(source, args.source_receipt, args.source_receipt_sha256)
    require(sha(checkpoint / "checkpoint.json") == args.checkpoint_sha256, "checkpoint identity differs")
    manifest = json.loads((checkpoint / "checkpoint.json").read_bytes())
    require(manifest["schema_version"] == 14 and manifest["pending_candidate"] is None and manifest["baseline_candidate"] == CANDIDATE, "checkpoint is not the acknowledged H16 baseline")
    ...
    require(SCHEMA_VERSION == args.target_schema and args.target_schema >= 27, "runtime schema differs from reviewed target")
    runtime_bound(source)
    destination.mkdir()
    hub = remote_reader(args.public_repo, destination / "remote-cache")
    state = destination / "restored-state"
    report: dict[str, Any] = dict(
        format="extension-operational-restore-rehearsal-v1", started_at=datetime.now(UTC).isoformat(), passed=False,
        checkpoint=str(checkpoint), checkpoint_sha256=args.checkpoint_sha256, acknowledged_archive_commit=args.archive_commit,
        source=str(source), source_receipt_sha256=args.source_receipt_sha256, helper_sha256=args.helper_sha256,
        public_repo=args.public_repo, baseline=BASELINE, state=str(state), units_before=before_units,
        target_schema=args.target_schema,
        source_requests=0, public_writes=0, input_acceptance=False, workers_started=False, repairs_activated=False,
    )
    try:
        ...
        checked = exercise_restore(checkpoint, state, manifest, hub, SystemClock(), maximum_schema=SCHEMA_VERSION)
        report["remote"] = checked.remote
        report["restore_heads"] = checked.heads
        with open_database(state, read_only=True) as database:
            require(database.schema_version == 14, "restore unexpectedly migrated before activation")
            require(util.table_receipts(database.connection) == report["before"], "restoration changed retained application state")
        report["schema14_activated_under_hold"] = True
        report["stage"] = "held_migration"
        checked.held()
        with open_database(state) as database:
            after = util.table_receipts(database.connection)
            report["changed_existing_tables"] = util.compare(report["before"], after)
            report["after"] = after
            report["schema"] = database.schema_version
            require(not report["changed_existing_tables"] and database.schema_version == SCHEMA_VERSION, "migration changed retained state")
            require([tuple(row) for row in database.connection.execute("PRAGMA integrity_check")] == [("ok",)], "migrated integrity failure")
            require(database.connection.execute("PRAGMA foreign_key_check").fetchone() is None, "migrated foreign-key failure")
        checked.held()
        with open_database(state) as database:
            require(util.table_receipts(database.connection) == after, "reopening changed migrated state")
        report["stage"] = "preservation_and_remote_recheck"
        for name, record in manifest["files"].items():
            if name != "state.sqlite":
                require(sha(state / name) == record["sha256"], f"restored private file changed: {name}")
        require(sha(checkpoint / "checkpoint.json") == args.checkpoint_sha256 and sha(checkpoint / "state.sqlite") == manifest["files"]["state.sqlite"]["sha256"], "checkpoint changed during rehearsal")
