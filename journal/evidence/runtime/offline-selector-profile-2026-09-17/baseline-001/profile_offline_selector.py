...
def main() -> None:
    ...
    require(marker['format']=='extension-input-scratch-v1' and marker['scratch']==str(scratch) and marker['source']==str(source) and marker['source_receipt_sha256']==SOURCE_SHA and marker['schema']==28,"scratch source/schema identity differs")
    ...
    for name,module in tuple(sys.modules.items()):
        if name=='swingset' or name.startswith('swingset.'):
            ...
            require(isinstance(location,str) and Path(location).resolve().is_relative_to(source/'src'),"runtime import escaped frozen003")
