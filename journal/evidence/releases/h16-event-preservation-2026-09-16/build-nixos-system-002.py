...
OUTPUT=EVIDENCE/'nix-build-002.json'
...
require(not OUTPUT.exists() and not (EVIDENCE/'nix-build-002.log').exists(),'Build evidence must be new')
require(sha(SOURCE/'h16-source.json')==SOURCE_HASH,'Frozen receipt mismatch')
argv=['nix','--extra-experimental-features','nix-command flakes','build','--impure','--offline','--no-update-lock-file','--no-link','--print-out-paths',f'path:{SOURCE}#nixosConfigurations.orb.config.system.build.toplevel']
...
try:
    ...
    (EVIDENCE/'nix-build-002.log').open('x').write(result.stdout+result.stderr)
