...
def memory_sample(current,guard,unit,sample):
    ...
    if group:
        try:
            ...
            after=status(unit)
            require(after.get('MainPID')=='0' and (after.get('ActiveState') in ('inactive','failed') or after.get('LoadState')=='not-found'),'live worker lost cgroup telemetry')
            sample.update(after);sample['telemetry']='cgroup_removed_after_exit'
            return None
    ...
    try:
        with (capture/'systemd-wait.log').open('xb') as log:
            ...
            while process.poll() is None:
                ...
                memory=memory_sample(current,guard,unit,sample)
