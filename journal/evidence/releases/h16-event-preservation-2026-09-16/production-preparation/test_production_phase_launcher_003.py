...
SPEC=importlib.util.spec_from_file_location('phase_launcher',Path(__file__).with_name('run-production-phase-003.py'))
...
def test_missing_live_cgroup_rejected(pid,active,substate,group):
    with pytest.raises(ValueError,match='cgroup'):
        module.memory_sample({'MainPID':pid,'ActiveState':active,'SubState':substate,'ControlGroup':group},SimpleNamespace(),'worker.service',{})
...
def test_memory_threshold_stops_at_limit(value):
    with pytest.raises(ValueError,match='reached'):
        module.memory_sample({'MainPID':'1','ControlGroup':'/test'},SimpleNamespace(anonymous_bytes=lambda _:value),'worker.service',{})
...
def test_cgroup_exit_race_rechecks_exact_unit(monkeypatch,terminal):
    def vanished(_):raise FileNotFoundError('cgroup exited')
    seen=[]
    def state(unit):
        seen.append(unit)
        return {'MainPID':'0' if terminal else '42','ActiveState':'inactive' if terminal else 'active'}
    monkeypatch.setattr(module,'status',state)
    sample={};current={'MainPID':'42','ControlGroup':'/worker'}
    if terminal:
        assert module.memory_sample(current,SimpleNamespace(anonymous_bytes=vanished),'exact.service',sample) is None
        assert sample['telemetry']=='cgroup_removed_after_exit'
    else:
        with pytest.raises(ValueError,match='lost cgroup'):module.memory_sample(current,SimpleNamespace(anonymous_bytes=vanished),'exact.service',sample)
    assert seen==['exact.service']
