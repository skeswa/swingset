...
def main():
    ...
    active=subprocess.run(['systemctl','list-units','--all','--state=active,activating,deactivating','--no-legend','--plain','swingset-h16*.service'],capture_output=True,text=True,check=True,timeout=15)
    require(not active.stdout.strip(),'another H16 unit is active')
    ...
    report={'phase':args.phase,'started_at':datetime.now(UTC).isoformat(),'argv':argv,'script_sha256':sha(Path(__file__)),'driver_sha256':sha(driver),'gate_sha256':args.gate_sha256,'passed':False}
    ...
    try:
        with (capture/'systemd-wait.log').open('xb') as log:
            ...
            while process.poll() is None:
                ...
                if group and current.get('MainPID','0')!='0':
                    seen=True;memory=Path('/sys/fs/cgroup')/group.lstrip('/')/'memory.stat'
                    require(memory.is_file(),'live memory telemetry missing')
                    values={line.split()[0]:int(line.split()[1]) for line in memory.read_text().splitlines()}
                    require('anon' in values,'anonymous telemetry missing')
                    sample['anonymous_bytes']=values['anon'];peak=max(peak,values['anon'])
                    require(values['anon']<=6*1024**3,'anonymous memory above 6GiB')
                else:
