...
spec = importlib.util.spec_from_file_location('initialization_memory_guard', HERE / 'guard-initialization-003.py')
guard = importlib.util.module_from_spec(spec)
spec.loader.exec_module(guard)


def setup(tmp_path, monkeypatch, states, *, anon=10, results=('current',)):
    supervisor = guard.load_supervisor(HERE.parent / 'supervise-initialization-002.py')
