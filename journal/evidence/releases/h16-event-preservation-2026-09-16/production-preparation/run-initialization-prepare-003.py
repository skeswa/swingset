"""Retry prepare with the reviewed staged authority and unchanged marker; never run."""
import hashlib
import json
import os
import pwd
...
CAPTURE = Path('/var/tmp/h16-event-preservation-initialization-prepare-003-20260916')
UNIT = 'swingset-h16-event-preservation-initialization-prepare-003.service'
...
require(sha(OPS / 'initialization-gate-002.json') == GATE_SHA, 'approved gate changed')
require(sha(OPS / 'initialization-prepare-review-002.json') == REVIEW_SHA, 'approved review changed')
require(sha(OPS / 'initialize-002.py') == DRIVER_SHA, 'reviewed initializer changed')
gate = json.loads((OPS / 'initialization-gate-002.json').read_bytes())
review = json.loads((OPS / 'initialization-prepare-review-002.json').read_bytes())
...
MARKER_SHA = 'dad425983041d0a27ab2eae1127db1fc9aeec4a401f50d678d6fe87e176baece'
owner = pwd.getpwnam('swingset')
for name, digest in [('initialization-gate-002.json',GATE_SHA),('initialization-prepare-review-002.json',REVIEW_SHA),('initialization-marker-002.json',MARKER_SHA), *[(binding['target'],binding['sha256']) for binding in HELPERS.values()]]:
    path = OPS / name
    require(not path.is_symlink() and path.is_file() and sha(path) == digest, 'staged authority changed: ' + name)
    info = path.stat()
    require(info.st_uid == owner.pw_uid and info.st_gid == owner.pw_gid and info.st_mode & 0o777 == 0o600, 'staged authority permissions differ: ' + name)
require(not (OPS / 'prepare-003.json').exists() and not (OPS / 'prepare-003.json').is_symlink(), 'new prepare output already exists')
...
worker = ['/var/lib/swingset/venv/bin/python', str(OPS / 'initialize-002.py'), 'prepare', '--gate', str(OPS / 'initialization-gate-002.json'), '--marker', str(OPS / 'initialization-marker-002.json'), '--output', str(OPS / 'prepare-003.json')]
...
try:
    ...
    result = json.loads((OPS / 'prepare-003.json').read_bytes()); marker = json.loads((OPS / 'initialization-marker-002.json').read_bytes())
    ...
    require(result['marker_sha256'] == sha(OPS / 'initialization-marker-002.json') == MARKER_SHA, 'immutable marker hash differs')
    require(marker['input_bundle_hash'] == gate['input_bundle_hash'] and marker['gate_sha256'] == GATE_SHA, 'marker authority differs')
    held()
    report.update(passed=True, prepare_sha256=sha(OPS / 'prepare-003.json'), marker_sha256=sha(OPS / 'initialization-marker-002.json'), changed_inputs=result['changed_inputs'], extract_cache_invalidated=result['extract_cache_invalidated'])
    ...
    for name in ('prepare-003.json', 'initialization-marker-002.json'):
        path = OPS / name
        if path.is_file():
