...
OUT = Path('/Users/skeswa/repos/skeswa/swingset/journal/evidence/releases/h16-validation-2026-09-16/event-ownership-diagnostic-003.json')
...
try:
    ...
    for generation in private['selected']:
        ...
        for row in db.execute("SELECT record_key,payload_json FROM derivation_rows WHERE generation_id=? AND table_name='events'",(generation['generation_id'],)):
            ...
            event_outputs.setdefault(event,[]).append(info)
    ...
    state_events={r['event_id']:dict(r) for r in db.execute('SELECT event_id,series_id,event_month,held,history_source FROM events')}
    changed=[{'event_id':k,'baseline_series':v['series_id'],'baseline_month':v['event_month'],'state':state_events.get(k),'owners':[{'kind':x['generation']['unit_kind'],'generation_id':x['generation']['generation_id'],'series_id':x['payload'].get('series_id'),'event_month':x['payload'].get('event_month'),'support':x['support_state'],'legacy_matches':x['legacy_matches']} for x in event_outputs.get(k,[])]} for k,v in old.items() if v['series_id'].startswith('wsdc-') and (state_events.get(k,{}).get('series_id')!=v['series_id'] or state_events.get(k,{}).get('event_month')!=v['event_month'])]
    report['registry_identity_changed_count']=len(changed)
    report['registry_identity_changed_examples']=changed[:12]
    report['registry_identity_changed_in_public']=sum(x['event_id'] in present for x in changed)
    report['rejected_events_with_another_admissible_base']=sum(any(x['payload'].get('snapshot_id') and (x['support_state']=='accepted' or x['legacy_matches']) for x in event_outputs[e]) for e in rejectedids)
    report['rejected_events_without_another_admissible_base']=[e for e in rejectedids if not any(x['payload'].get('snapshot_id') and (x['support_state']=='accepted' or x['legacy_matches']) for x in event_outputs[e])]
    report['changed_public_examples']=[x for x in changed if x['event_id'] in present][:5]
    report['calendar_rejected_examples']=[{k:v for k,v in x.items() if k!='generation'} for x in rejected if x['generation']['unit_kind']=='calendar']
    report['missing_judges_without_observation_snapshot']=[]
    for row in duck.execute(f"SELECT j.judge_id,j.event_id,j.name_raw,j.snapshot_id FROM read_parquet('{OLD}/data/judges/*.parquet',hive_partitioning=false) j LEFT JOIN read_parquet('{NEW}/data/judges/*.parquet',hive_partitioning=false) n USING(judge_id) WHERE n.judge_id IS NULL").fetchall():
        if not db.execute('SELECT 1 FROM observations WHERE snapshot_id=? LIMIT 1',(row[3],)).fetchone(): report['missing_judges_without_observation_snapshot'].append(row)
    report['passed']=True
