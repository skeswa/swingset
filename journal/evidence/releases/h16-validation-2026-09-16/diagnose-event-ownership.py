...
OUT = Path('/Users/skeswa/repos/skeswa/swingset/journal/evidence/releases/h16-validation-2026-09-16/event-ownership-diagnostic.json')
...
try:
    ...
    oldrows=duck.execute(f"SELECT * FROM read_parquet('{OLD}/data/events/*.parquet',hive_partitioning=false)")
    names=[c[0] for c in oldrows.description]
    old={r['event_id']: _normalize(r) for r in (dict(zip(names,v)) for v in oldrows.fetchall())}
