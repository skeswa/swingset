"""Assemble the narrow original-H16 proof fix; never build or activate it.
...
BASE_RECEIPT_SHA256 = "f8f24c2d8b839bbfcd25bf87e551e9f2777d1236f89ed84da16c3c07d4d75b9f"
RUNTIME = (
...
REVIEWED = (*RUNTIME, TEST, "pyproject.toml")
PRIOR_RECEIPT = "provenance/h16-before-proof-validation.json"
...
def pytest_only(base_body, reviewed_body):
    ...
    reviewed = tomllib.loads(reviewed_body.decode())
    wanted = reviewed["tool"]["pytest"]["ini_options"]["pythonpath"]
    require(wanted == ["src", ".", "tests"], "Unexpected reviewed pytest pythonpath")
    before = tomllib.loads(base_text)
    require(
        before["tool"]["pytest"]["ini_options"]["pythonpath"] in (["src"], ["src", "."]),
        "Unexpected base pytest pythonpath",
    )
    pattern = r"(?m)^pythonpath = \[[^\n]*\]$"
    patched, count = re.subn(pattern, 'pythonpath = ["src", ".", "tests"]', base_text)
    require(count == 1, "Base pytest pythonpath changed or is not unique")
    after = tomllib.loads(patched)
    before["tool"]["pytest"]["ini_options"]["pythonpath"] = wanted
    require(before == after, "Unexpected pyproject changes")
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("/var/tmp/swingset-h16-proof-release-source")
    )
    parser.add_argument("--reviewed-files", type=Path, required=True)
    args = parser.parse_args()
    base, work, output = args.base, args.work, args.output
    require(base.resolve() == BASE, "Only the exact frozen 2d5 source is allowed")
    ...
    require(RUNTIME[0] not in before and TEST not in before, "New files already exist in base")
    require(
    ...
    receipt = {
        ...
        "purpose": "Original H16 transaction-scoped proof reuse with unchanged 45-second completion and source policies",
        "pyproject_change": "Only adopt reviewed pytest pythonpath; preserve base metadata and dependencies",
