...
state = Path('/var/tmp/swingset-h16-closure-completion-diagnostic')
output = Path('/var/tmp/swingset-h16-closure-completion-diagnostic.json')
...
report = {'format': 'h16-completion-diagnostic-v1', 'state': str(state), 'predecessor': str(source), 'closure_file_sha256': hashlib.sha256(patch.read_bytes()).hexdigest(), 'diagnostic_only': True, 'production_mutated': False, 'inputs_accepted': False, 'network_requests': 0, 'passed': False}
