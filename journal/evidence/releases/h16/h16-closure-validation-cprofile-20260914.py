...
try:
    ...
    profiler.disable()
    profiler.dump_stats("/var/tmp/h16-closure-optimized-cprofile-20260914.prof")
    stats = pstats.Stats(profiler)
    report["profile_top_cumulative"] = [{"function":str(k),"calls":v[1],"self_seconds":v[2],"cumulative_seconds":v[3]} for k,v in sorted(stats.stats.items(), key=lambda item:item[1][3], reverse=True)[:25]]
