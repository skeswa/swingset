...
overlay_hashes = {str(replacement):hashlib.sha256(replacement.read_bytes()).hexdigest()}
for short in ('derivation_signatures','derivation_readiness','derivations'):
    path=Path('/Users/skeswa/repos/skeswa/swingset/src/swingset/state')/(short+'.py')
    current_spec=importlib.util.spec_from_file_location('swingset.state.'+short,path)
    current_module=importlib.util.module_from_spec(current_spec)
    sys.modules[current_spec.name]=current_module
    current_spec.loader.exec_module(current_module)
    setattr(sys.modules['swingset.state'],short,current_module)
    overlay_hashes[str(path)]=hashlib.sha256(path.read_bytes()).hexdigest()
...
result={'source':str(source),'state':str(state),'read_only':True,'network_requests':0,'worker_executed':False,'overlay_hashes':overlay_hashes,'overlay_file':str(replacement),'overlay_sha256':hashlib.sha256(replacement.read_bytes()).hexdigest()}
...
Path('/var/tmp/swingset-h16-link-selection-bulk.json').write_text(json.dumps(result,indent=2,sort_keys=True)+'\n')
