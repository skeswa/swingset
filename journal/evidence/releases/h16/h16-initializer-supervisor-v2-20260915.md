# H16 initializer supervisor v2 — offline preparation only
...
- Supervisor: `/tmp/h16-initializer-supervisor-v2.py`, SHA256 `2f6c6f88604db55342bc9d3adbfee9b789143846815cc6380768d26ebb952cae`.
- Unchanged initializer: `/tmp/h16-live-initialize.py`, SHA256 `b54c02a15c3d86a603bef4d6926e66bde4cf1ef9d23f273aa5db771d3a99d233`.
- Tests: `/tmp/test_h16_initializer_supervisor_v2.py`; actual result: **44 passed in 0.20s**. Log: `/tmp/h16-initializer-supervisor-v2-full-tests.log`.
...
V2 changes only the launch cleanup boundary: the batch starts with `process=None`, launch runs inside its try/finally, and interruption or launch failure always stops the known unit. Cleanup waits on a process only if launch returned one. Both a started-unit interruption and an ordinary launch error with no active unit retain batch-final and final stopped evidence. All v1 files remain unchanged; the v1 launch-gap regression was adapted only to import v2 and now passes alongside the full supervisor suite.
...
sudo env \
  ...
  /var/lib/swingset/venv/bin/python /tmp/h16-initializer-supervisor-v2.py \
...
/Users/skeswa/repos/skeswa/swingset/.venv/bin/pytest -q /tmp/test_h16_initializer_supervisor_v2.py /tmp/test_h16_supervisor_launch_review_v2.py
```
