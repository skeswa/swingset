...
SOURCE_RECEIPT_SHA = "60cdfe64004a0ba5a9aee207bcb089d6d4c81f5ca58169ccdb88a5dfab8146d6"
DRIVER = "dcn-score-pdf-lookup-h13-001.py"
DRIVER_SHA = "b3e50815456113d44eb96885199c4d6caddef65d408a7ee9d85e3803cb1dd80d"
HELPER_SHA = "2ad23ab7bfd923bf0d9fe7afed49b1410867c187f8f473aeefb73d427c1a1059"
MANIFEST_SHA = "f25df5f025ccd81ed95c6e0a87ecb952ff7fd26baceb064b68c512bafb6711ac"
...
def verify_runtime(source: Path) -> int:
    ...
    if sha(receipt) != SOURCE_RECEIPT_SHA:
        raise ValueError("frozen schema28 source receipt changed")
...
def inspect_state(state: Path, *, now: datetime) -> dict[str, Any]:
    ...
    with sqlite3.connect((state / "state.sqlite").as_uri() + "?mode=ro", uri=True) as conn:
        ...
        if schema is None or int(schema[0]) != 28 or int(conn.execute('PRAGMA user_version').fetchone()[0]) != 28:
            raise ValueError("exact schema28 required; this helper never migrates")
    ...
    with (state / "state.lock").open("r+b") as lock:
        ...
        authorization = dict(
            ...
            execution_window=dict(
                starts_at=now.isoformat(), expires_at=(now + timedelta(minutes=15)).isoformat()
            ),
        )
        gate = dict(
