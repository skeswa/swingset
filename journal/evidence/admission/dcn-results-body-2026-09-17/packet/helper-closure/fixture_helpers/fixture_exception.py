...
def read_manifest(path: Path, repository: Path) -> dict[str, Any]:
    ...
    for target in manifest["targets"]:
        ...
        body = retained_evidence_path(repository, evidence["path"]).read_bytes()
        ...
        if headers := target.get("retained_headers"):
            if (
                digest(retained_evidence_path(repository, headers["path"]).read_bytes())
                != headers["sha256"]
            ):
...
def validate_authorization(
    ...
    try:
        ...
        valid = approved <= starts <= now < expires <= starts + timedelta(minutes=15)
    except (KeyError, TypeError, ValueError):
        valid = False
    if not valid:
        raise FixtureStopped(
            "execution window must be current, at most 15 minutes, and after approval"
        )
