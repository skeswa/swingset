...
REPO = Path(__file__).resolve().parents[1] / "helper-closure"
MANIFEST = REPO / "proposal.json"
exception = importlib.import_module("fixture_helpers.fixture_exception")
transport = importlib.import_module("fixture_helpers.fixture_transport")
...
def test_dry_run_does_not_open_state_or_request(tmp_path):
    output = subprocess.check_output(
        [
            sys.executable,
            "-m",
            "fixture_helpers.fixture_exception",
            "--manifest",
            str(MANIFEST),
            "--state",
            str(tmp_path / "absent"),
...
from pathlib import Path
import httpx
from fixture_helpers.fixture_exception import accounting_connection, read_manifest
