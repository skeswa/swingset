...
DRIVER = "fixture-exception-h13-002.py"
DRIVER_SHA = "81357b33c8862bdc4bdd2d84f2479a7913a9d783b00b435910cc2328bdcf69f7"
HELPER_SHA = "f8e0557026bc3bcfbd06624071bbc98ab7c7a46983fd6e3117d34140d6b16db3"
...
def verify_packet(packet: Path) -> Path:
    ...
    if {p.relative_to(helpers).as_posix() for p in children if p.is_file()} != set(receipt["files"]) | {"closure.json"}:
...
def inspect_state(state: Path, *, now: datetime) -> dict[str, Any]:
    ...
    with sqlite3.connect((state / "state.sqlite").as_uri() + "?mode=ro", uri=True) as conn:
        ...
        budget = conn.execute("SELECT requests,bytes FROM host_budget WHERE host='web.archive.org' AND day=?", (day,)).fetchone()
        ...
        return dict(schema=14, baseline_path=str(baseline),
                    published_sha256=sha(baseline / "PUBLISHED"),
                    manifest_sha256=sha(baseline / "_meta/manifest.json"),
                    archive_day=day, archive_requests=requests, archive_bytes=received,
                    archive_requests_remaining_under_200=max(0, 200 - requests),
                    host=dict(host) if host else None, operator_pauses=pauses,
                    pending_work=pending,
                    quota_basis="read-only preparation; actual gate rechecks and debits shared budget")


def prepare(*, packet: Path, state: Path, quarantine: Path, approved_by: str,
            approved_at: datetime, decision: str, publication: str, now: datetime) -> dict[str, Any]:
    packet, state, quarantine = packet.resolve(strict=True), state.resolve(strict=True), quarantine.resolve()
    ...
    if quarantine.exists() or any((packet / name).exists() for name in OUTPUTS):
        raise ValueError("single-use preparation or quarantine already exists; do not retry by changing paths")
    ...
    with (state / "state.lock").open("r+b") as lock:
        ...
        authorization = dict(
            ...
            manifest_canonical_sha256=MANIFEST_SHA, state=str(state), quarantine=str(quarantine),
            published_stages=["V2", "V4"], approved_by=approved_by,
            owner_decision_reference=decision, publication_receipt_reference=publication,
            approved_at=approved_at.isoformat(),
            approval_time_basis="coordinator-observed owner response; not an inferred click timestamp",
            execution_window=dict(starts_at=now.isoformat(), expires_at=(now + timedelta(hours=24)).isoformat()),
        )
        gate = dict(format="fixture-h13-coordinator-v2", driver_sha256=DRIVER_SHA,
                    helper_root=str(helpers), helper_manifest_sha256=HELPER_SHA,
                    source=str(SOURCE), source_receipt="h16-source.json",
                    source_receipt_sha256=SOURCE_RECEIPT_SHA, schema=14, state=str(state),
                    **{key: observed[key] for key in ("baseline_path", "published_sha256", "manifest_sha256")})
        ...
        command = ["env", "PYTHONDONTWRITEBYTECODE=1", f"PYTHONPATH={SOURCE / 'src'}:{SOURCE}",
                   "/var/lib/swingset/venv/bin/python", str(packet / DRIVER),
                   "--repo", str(SOURCE), "--manifest", str(helpers / "proposal.json"),
                   "--state", str(state), "--quarantine", str(quarantine),
                   "--authorization", str(packet / "authorization.json"),
                   "--execution-gate", str(packet / "execution-gate.json"),
                   "--execution-gate-sha256", gate_sha, "--execute"]
        receipt = dict(format="fixture-preparation-v1", prepared_at=now.isoformat(),
                       source_files_verified=source_files, gate_sha256=gate_sha,
                       authorization_sha256=sha(packet / "authorization.json"),
                       state_observation=observed, command=command,
                       executed=False, network_requests=0,
...
def main() -> None:
    ...
    os.umask(0o077)
    result = prepare(packet=args.packet, state=args.state, quarantine=args.quarantine,
                     approved_by=args.approved_by, approved_at=datetime.fromisoformat(args.approved_at),
                     decision=args.decision, publication=args.publication, now=datetime.now(UTC))
