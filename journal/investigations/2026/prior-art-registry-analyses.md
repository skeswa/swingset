...
| 2026 (swingset, `journal/investigations/undated/initial-source-survey.md`) | `POST /lookup2020/find`                              | `num=<wsdc_id>`                                   | `leader` and `follower` blocks, `dominate_role`, `dominate_required`, `dominate_allowed`; the 2024 `dominate_data` key is not in our verified sample                                 |
...
- Events can be recorded out of chronological order, and a combined
  ...
  divisions award the lower division's points (`docs/reference/wsdc-rules.md`)
...
| 2026-09-09 | 27,039       | mechstack dump (`journal/archive/v1-implementation-evidence.md`) |

About 1,000 new numbers a year before 2020, about 1,200 a year since 2022. This supports the sweep budget in `docs/reference/sources/wsdc-registry.md`
...
- **Rates.** Tseng ran 4 to 5 requests per second single-threaded and
  ...
  slowest. `journal/investigations/undated/initial-source-survey.md` already notes that scrapers at 3 per
...
- Add the rule-change history these posts document to
  `docs/reference/wsdc-rules.md` so consumers can split cohorts at the right
