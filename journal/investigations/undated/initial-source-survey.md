# Original source survey and response examples
...
lives in the playbooks under `docs/reference/sources/`, which win over this
...
precision ([backfill](../../../docs/reference/backfill.md#event-enumeration-for-history)).
...
- A paid REST API exists (RapidAPI, and `pointstest2.scoring.dance`
  ...
  at a fair price, it replaces scraping for this source. See [open questions](../../archive/initial-open-questions.md).
...
- The payload is a JavaScript function call, not JSON. We evaluate it in
  a sandboxed `node` subprocess ([parsing](../../../docs/reference/parsing.md#parsing-rules)) and fall back to nothing.
  We do not regex it.
...
- No index: bucket root, `robots.txt`, and sitemap are 403. Discovery is
  ...
  `docs/reference/sources/world-dance-registry.md`.
...
- Step Right Solutions: server returns empty 200s. Dead at the origin,
  ...
  archive-only source for the 2010 start (`docs/reference/sources/step-right-solutions.md`).
