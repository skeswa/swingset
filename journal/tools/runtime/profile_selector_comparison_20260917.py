...
SOURCE_SHA = "acfe304af71280843a2087b4e415118d81aaa8800a58e48620fbfe3ca018df41"
...
def measure(
    conn: sqlite3.Connection, selector: Any, *, seconds: float, derivations: Any, readiness: Any
) -> tuple[dict[str, Any], cProfile.Profile]:
    require(0 < seconds <= 30, "selection profile must be at most30 seconds")
    profile = cProfile.Profile()
    calls: dict[tuple[str, str, str], dict[str, Any]] = defaultdict(
        lambda: dict(calls=0, false=0, seconds=0.0)
    )
    ...
    require(
        ...
        "existing process timer must not be replaced",
    )
    ...
    report: dict[str, Any] = dict(
        ...
        baseline_query_only=conn.execute("PRAGMA query_only").fetchone()[0],
        instrumentation="cProfile and current/desired/ready counters; no SQL trace callback; not uninstrumented throughput",
    )
    ...
    try:
        ...
        unit = selector()
        report.update(
            status="selected" if unit else "no_runnable_unit",
            selected=None
            if unit is None
            else dict(stage=unit.stage, kind=unit.unit_kind, unit_id=unit.unit_id),
        )
        ...
        report["derivation_calls"] = [
            dict(function=name, stage=stage, kind=kind, **values)
            for (name, stage, kind), values in sorted(calls.items())
        ]
...
def main() -> None:
    ...
    require(
        not scratch.is_relative_to(Path("/var/lib/swingset").resolve())
        and "checkpoints" not in scratch.parts,
        "profiling is restricted to disposable scratch",
    )
    require(
        not output.exists()
        and not any(
            output == root or output.is_relative_to(root)
            for root in (scratch, source, Path("/var/lib/swingset").resolve())
        ),
        "new output must be outside scratch/source/production",
    )
    ...
    require(
        ...
        and marker["source"] == PARENT_SOURCE
        and marker["source_receipt_sha256"] == PARENT_SOURCE_SHA
        and marker["schema"] == 28,
        "scratch source/schema identity differs",
    )
    require(
        not (scratch / "RESTORE_PENDING").exists()
        and sha(scratch / "operator-hold") == marker["retained_files"]["operator-hold"]["sha256"],
    ...
    require(
        json.loads(receipt.read_bytes())["predecessor_receipt_sha256"] == PARENT_SOURCE_SHA,
        "selector derivative parent differs",
    )
    ...
    require(
        sha(verifier) == (expected["sha256"] if isinstance(expected, dict) else expected),
        "frozen verifier differs",
    )
    ...
    for name, module in tuple(sys.modules.items()):
        if name == "swingset" or name.startswith("swingset."):
            ...
            require(
                ...
                "runtime import escaped reviewed selector derivative",
            )
    ...
    with (scratch / "state.lock").open("r+b") as lock:
        ...
        require(
            not (scratch / "RESTORE_PENDING").exists()
            and sha(scratch / "operator-hold")
            == marker["retained_files"]["operator-hold"]["sha256"],
            "scratch hold changed at writer lock",
        )
        conn = sqlite3.connect(
            (scratch / "state.sqlite").as_uri() + "?mode=ro", uri=True, isolation_level=None
        )
        try:
            ...
            report, profile = measure(
                conn,
                lambda: next_offline(
                    conn, now=now, allowed=lambda unit: unit_allowed(conn, unit, now=now)
                ),
                seconds=args.seconds,
                derivations=derivations,
                readiness=derivation_readiness,
            )
            require(
                conn.total_changes == 0
                and conn.execute("PRAGMA data_version").fetchone()[0] == before,
                "scratch database changed during profile",
            )
        finally:
            conn.close()
    util.verify_source(source, receipt, SOURCE_SHA)
    ...
    report.update(
        format="reviewed-selector-comparison-profile-v1",
        parent_source_receipt_sha256=PARENT_SOURCE_SHA,
        ...
        data_version_unchanged=True,
        worker_executions=0,
        network_requests=0,
        production_applied=False,
        scope="one read-only instrumented selection; source and interlock checks occur outside selection timeout",
    )
