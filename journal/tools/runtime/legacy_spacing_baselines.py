...
def settled(conn: sqlite3.Connection) -> None:
    require(
        ...
        is None,
        "unsettled execution/control admissions remain",
    )
...
def fixture_evidence(root: Path, prior_hosts: Path) -> dict[str, Any]:
    ...
    admissions = [
        row
        for row in json.loads((root / "execution-admissions.json").read_bytes())
        if row["action_kind"] == "request"
    ]
    requests = receipt["requests"]
    require(
        len(requests) == len(admissions) == 8 and receipt["received_bytes"] == 130059,
        "unexpected fixture request accounting",
    )
    require(
        preparation["state_observation"]["archive_requests"]
        == preparation["state_observation"]["archive_bytes"]
        == 0,
        "fixture did not begin with its proven zero daily debit",
    )
    bodies = {}
    for request in requests:
        ...
        require(
            request["complete"] is True
            and sha(body) == digest
            and body.stat().st_size == request["body_bytes"],
            "fixture body is missing or changed",
        )
        bodies[digest] = request["body_bytes"]
    robots = [row for row in requests if row["purpose"] == "robots"]
    require(
        len(robots) == 1 and robots[0]["url"] == f"https://{HOST}/robots.txt",
        "robots request identity differs",
    )
    robot = robots[0]
    policy = _policy(
        robot["url"],
        robot["http_status"],
        (root / "quarantine/bodies" / robot["body_sha256"]).read_bytes(),
    )
    old_floor = parse_hosts(prior_hosts.read_bytes())[HOST].min_gap_seconds
    gap = max(5.0, 10.0, old_floor, policy.crawl_delay)
    require(
        policy.allowed and gap == 10 and manifest["limits"]["minimum_gap_seconds"] == 10,
        "fixture's original gap is not the reviewed ten seconds",
    )
    return dict(
        kind="archive-fixture-2026-09-17-eight-requests",
        files=FIXTURE_FILES,
        prior_hosts_sha256=PRIOR_HOSTS_SHA,
        original_effective_gap_seconds=gap,
        robots=dict(
            body_sha256=robot["body_sha256"],
            http_status=robot["http_status"],
            issued_at=robot["issued_at"],
            crawl_delay=policy.crawl_delay,
        ),
        bodies=bodies,
        started_at=receipt["started_at"],
        finished_at=receipt["finished_at"],
        day=preparation["state_observation"]["archive_day"],
        requests=8,
        received_bytes=130059,
...
def dcn_evidence(root: Path, previous: dict[str, Any]) -> dict[str, Any]:
    ...
    require(
        previous["kind"] == "archive-fixture-2026-09-17-eight-requests",
        "DCN proof needs its exact predecessor",
    )
    ...
    require(
        (preparation["archive_day"], preparation["archive_requests"], preparation["archive_bytes"])
        == (previous["day"], previous["requests"], previous["received_bytes"]),
        "DCN predecessor paid usage differs",
    )
    require(
        datetime.fromisoformat(receipt["started_at"])
        > datetime.fromisoformat(previous["finished_at"]),
        "DCN order differs",
    )
    requests = receipt["requests"]
    require(
        len(requests) == 2
        and receipt["received_bytes"] == 2804642
        and manifest["limits"]["minimum_gap_seconds"] == 10,
        "DCN scope differs",
    )
    bodies = {}
    for request in requests:
        ...
        require(
            request["complete"] is True
            and request["request_day"] == previous["day"]
            and sha(body) == digest
            and body.stat().st_size == request["body_bytes"],
            "DCN retained response differs",
        )
        require(
            request["spacing_basis"] == "closed_exchange_completion_floor"
            and request["next_dispatch_not_before_elapsed_seconds"]
            >= request["exchange_completed_elapsed_seconds"] + 10,
            "DCN original completion gap differs",
        )
        bodies[digest] = request["body_bytes"]
    robot, body_request = requests
    require(
        robot["purpose"] == "robots" and robot["url"] == f"https://{HOST}/robots.txt",
        "DCN robots identity differs",
    )
    require(
        body_request["transport_dispatched_elapsed_seconds"]
        >= robot["next_dispatch_not_before_elapsed_seconds"],
        "DCN actual dispatch preceded original gap",
    )
    policy = _policy(
        robot["url"],
        robot["http_status"],
    ...
    require(
        len(admissions) == 10
        and {row["action_id"]: row for row in admissions if row["action_id"] in predecessor}
        == predecessor,
        "DCN admission predecessor differs",
    )
    require(
        all(row["host"] == HOST and row["state"] == "settled" for row in admissions),
        "DCN admissions are not settled Archive requests",
    )
    usage = [row for row in after["archive_budget"] if row["day"] == previous["day"]]
    require(
        usage == [dict(host=HOST, day=previous["day"], requests=10, bytes=2934701)],
        "DCN retained cumulative ledger differs",
    )
    return dict(
        kind="archive-fixtures-2026-09-17-ten-requests",
        previous=previous,
        files=DCN_FILES,
        original_effective_gap_seconds=gap,
        robots=dict(
...
def assess_archive(conn: sqlite3.Connection, evidence: dict[str, Any]) -> str | None:
    row = conn.execute(
        "SELECT requests,bytes FROM host_budget WHERE host=? AND day=?", (HOST, evidence["day"])
    ).fetchone()
    if not row or tuple(row) != (evidence["requests"], evidence["received_bytes"]):
        return "paid usage differs from the retained fixture run"
    if conn.execute(
        "SELECT 1 FROM host_budget WHERE host=? AND day>? AND requests>0 LIMIT 1",
        (HOST, evidence["day"]),
    ).fetchone():
        return "later paid requests lack a reviewed original-gap proof"
    rows = conn.execute(
        "SELECT * FROM execution_admissions WHERE host=? AND action_kind='request' AND (julianday(admitted_at)>=julianday(?) OR julianday(admitted_at) IS NULL) ORDER BY action_id LIMIT ?",
        (HOST, evidence["started_at"], len(evidence["admissions"]) + 1),
    )
...
def prepare(conn: sqlite3.Connection, config: Any, evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in conn.execute(
        "SELECT * FROM hosts WHERE EXISTS(SELECT 1 FROM host_budget b WHERE b.host=hosts.host AND b.requests>0) ORDER BY host"
    ):
        host = row[0]
        policy = asdict(config.host(host))
        reason = (
            assess_archive(conn, evidence)
            if host == HOST
            else "no reviewed last-request, original-policy and robots proof"
        )
        items.append(
            dict(
                host=host,
                ...
                gap_seconds=max(
                    5.0, policy["min_gap_seconds"], evidence["original_effective_gap_seconds"]
                )
                if reason is None
                else None,
    ...
    return dict(
        format="reviewed-legacy-spacing-proposal-v1",
        schema=schema,
        hosts=items,
        evidence=evidence,
        protected=protected(conn),
    )
...
def apply(
    ...
    *,
    ...
    require(
        int(conn.execute("SELECT value FROM meta WHERE key='schema_version'").fetchone()[0]) == 28,
        "apply requires exact schema28 without migration",
    )
    ...
    for row in chosen:
        prior = conn.execute(
            "SELECT reservation_id,gap_seconds FROM host_request_spacing WHERE host=?",
            (row["host"],),
        ).fetchone()
        if prior and prior[1] is not None:
            existing = conn.execute(
                "SELECT evidence_ref,gap_seconds FROM host_request_spacing_baselines WHERE reservation_id=? AND host=?",
                (prior[0], row["host"]),
            ).fetchone()
            require(
                existing is not None and tuple(existing) == (reference, row["gap_seconds"]),
                "host already has different spacing authority",
            )
            results[row["host"]] = prior[0]
            continue
        results[row["host"]] = gate.establish_spacing_baseline(
            row["host"],
    ...
    require(
        protected(conn) == proposal["protected"],
        "baseline changed protected hosts, budgets or controls",
    )
    return results


def authorizer(
    action: int, table: str | None, column: str | None, database: str | None, origin: str | None
) -> int:
    allowed = {
        (sqlite3.SQLITE_INSERT, "host_request_spacing_baselines"),
        (sqlite3.SQLITE_INSERT, "host_request_spacing"),
        (sqlite3.SQLITE_UPDATE, "host_request_spacing"),
        (sqlite3.SQLITE_INSERT, "hosts"),
    }
    if action in {sqlite3.SQLITE_INSERT, sqlite3.SQLITE_UPDATE, sqlite3.SQLITE_DELETE}:
        return (
            sqlite3.SQLITE_OK
            if origin is None and (action, table) in allowed
...
def main() -> None:
    ...
    require(
        sha(source / name) == (entry["sha256"] if isinstance(entry, dict) else entry),
        "frozen source helper differs",
    )
    ...
    bindings = dict(
        ...
        hosts_sha256=args.hosts_sha256,
        sources_sha256=args.sources_sha256,
        fixture_bundle=str(args.fixture_bundle.resolve()),
        prior_hosts=str(args.prior_hosts.resolve()),
    ...
    def inputs() -> tuple[Any, dict[str, Any]]:
        require(
            sha(args.config / "hosts.toml") == args.hosts_sha256
            and sha(args.config / "sources.toml") == args.sources_sha256,
    ...
    if args.mode == "apply":
        require(
            args.coordinator_reviewed
            and args.proposal is not None
            and args.proposal_sha256 is not None,
            "apply needs a concrete coordinator-reviewed proposal pin",
        )
        require(sha(args.proposal) == args.proposal_sha256, "proposal differs")
        packet = json.loads(args.proposal.read_bytes())
        require(
            packet["format"] == "reviewed-legacy-spacing-proposal-v1"
            and packet["schema"] in {14, 28},
            "unknown proposal format or schema",
        )
            ...
            if packet is None:
                result = dict(
                    **proposal,
                    ...
                    prepared_at=stopped.isoformat(),
                    applied=False,
                )
            else:
                require(
                    held.schema_version == 28 and packet["guards"] == before,
                    "apply schema or hold changed",
                )
                conn = sqlite3.connect(
                    (args.state.resolve() / "state.sqlite").as_uri() + "?mode=rw",
                    uri=True,
                ...
                try:
                    ...
                    identifiers = apply(
                        ...
                        stopped_at=datetime.fromisoformat(packet["stopped_at"]),
                        clock=clock,
                    )
                    runtime_bound(source)
                    require(
                        guards(args.state) == before,
                        "hold or ordinary services changed during apply",
                    )
                    inputs()
                    util.verify_source(source, args.source_receipt, args.source_receipt_sha256)
                    conn.commit()
                    ...
                    conn.close()
                result = dict(
                    format="legacy-spacing-application-v1",
                    applied=True,
                    proposal_sha256=args.proposal_sha256,
                    bindings=bindings,
                    baseline_ids=identifiers,
