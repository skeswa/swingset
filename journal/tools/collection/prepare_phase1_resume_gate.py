...
def assemble_gate(
    ...
    return {
        ...
        "original_targets": {
            ...
            for t in targets
            if t.target_id in driver.TARGET_IDS
        },
        "target_statuses": {
...
def main() -> None:
    ...
    with db.open_database(state, read_only=True, lock=True) as database:
        ...
        gate = assemble_gate(
            ...
            driver,
            args.driver,
            catalog.load_catalog(state / "phase1-catalog.json"),
            now=datetime.now(UTC),
        )
        driver.check_authority(database.connection, state, gate)
        driver.check_targets(
            database.connection, state, gate, catalog.load_catalog, sources.get_page_kind
        )
    ...
    print(
        json.dumps(
            {"gate_sha256": sha(args.output), "remaining": len(gate["remaining_target_ids"])}
        )
    )
