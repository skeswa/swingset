            ...
            output.append(
                {
                    ...
                    "parser": "wdr.rounds",
                    "notes": f"seeded from journal/evidence/collection/source-survey-2026-09-04/results-sources.csv; confidence={row.get('confidence', '')}",
                }
            )
    return output
...
def main() -> None:
    ...
    writer.writerows(
        rows(
            ...
            / "evidence/collection/source-survey-2026-09-04/results-sources.csv"
        )
    )
