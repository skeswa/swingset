...
DCN_SCORE_LOOKUP_PACKET selects a fresh built packet. PYTHONPATH must place its helper
...
PACKET = Path(os.environ["DCN_SCORE_LOOKUP_PACKET"]).resolve()
...
driver = module("dcn_score_lookup_driver", PACKET / "dcn-score-pdf-lookup-h13-001.py")
preparation = module("dcn_score_lookup_preparer", PACKET / "prepare_fixture_exception.py")
HOST = "web.archive.org"
MANIFEST = read_manifest(PACKET / "helper-closure/proposal.json", PACKET)
QUERIES = MANIFEST["metadata_queries"]
QUERY = QUERIES[0]
...
def setup(tmp_path):
    ...
    authorization = dict(
        ...
        owner_decision_reference=MANIFEST["approval_reference"],
...
def test_exact_five_request_metadata_plan_has_no_body_targets(setup):
    ...
    with driver.accounting_connection(state) as conn:
        ...
        assert [item["url"] for item in result["requests"]] == [
            ...
            *[q[key] for q in QUERIES for key in ("probe_url", "page0_url")],
        ]
        assert result["targets"] == []
        assert [r["further_pages_unexamined"] for r in result["metadata_results"]] == [1, 1]
        assert [r["pages_reported"] for r in result["metadata_results"]] == [2, 2]
        assert [r["original_url"] for r in result["metadata_results"]] == [
            q["original_url"] for q in QUERIES
        ]
        for table in [
            "watches",
            "snapshots",
        ...
        assert conn.execute("SELECT sum(requests),sum(bytes) FROM host_budget").fetchone()[0] == 5
        assert (
...
def test_empty_probe_never_requests_page0(setup, probe):
    ...
    assert [r["pages_reported"] for r in result["metadata_results"]] == [0, 0]
    assert all("page0_sha256" not in row for row in result["metadata_results"])
...
def test_dry_run_needs_no_state_or_owner_record(tmp_path):
    result = subprocess.run(
        [
            sys.executable,
            str(PACKET / "dcn-score-pdf-lookup-h13-001.py"),
            "--manifest",
    ...
    assert len(report["metadata_queries"]) == 2 and not list(tmp_path.iterdir())
...
def test_preparation_pins_schema28_and_preserves_state_without_network(
    ...
    result = preparation.prepare(
        ...
        decision=MANIFEST["approval_reference"],
...
def test_cached_robots_avoids_redundant_request(setup):
    ...
    with driver.accounting_connection(state) as conn:
        result = make(setup, conn).run()
    assert len(result["requests"]) == 4 and result["cached_robots"]["body_sha256"] == sha
...
def test_future_robots_cache_does_not_replace_fresh_policy(setup):
    ...
    with driver.accounting_connection(state) as conn:
        result = make(setup, conn).run()
    assert len(result["requests"]) == 5 and "cached_robots" not in result
...
def test_empty_first_url_still_runs_only_second_exact_query(setup):
    def reply(request):
        if request.url.path == "/robots.txt":
            return response(404)
        if request.url.params.get("showNumPages") == "true":
            return response(
                body=b"0" if request.url.params["url"] == QUERIES[0]["original_url"] else b"1"
            )
        return handler(request)

    with driver.accounting_connection(setup[0]) as conn:
        result = make(setup, conn, reply).run()
    assert result["status"] == "captured_pending_independent_review"
    ...
    assert [r["pages_reported"] for r in result["metadata_results"]] == [0, 1]
    assert "page0_sha256" not in result["metadata_results"][0]


def test_malformed_second_probe_preserves_first_evidence_and_stops(setup):
    def reply(request):
        if request.url.params.get("url") == QUERIES[1]["original_url"]:
            assert request.url.params.get("showNumPages") == "true"
            return response(body=b'{"pages":"changed"}')
        return handler(request)

    with driver.accounting_connection(setup[0]) as conn:
        result = make(setup, conn, reply).run()
        assert conn.execute("SELECT SUM(requests) FROM host_budget").fetchone()[0] == 4
    assert result["status"] == "stopped_incomplete" and len(result["requests"]) == 4
    first, second = result["metadata_results"]
    assert first["status"] == "captured_pending_independent_review" and "page0_sha256" in first
    assert (
        second["status"] == "pending" and "probe_sha256" in second and "page0_sha256" not in second
    )
    assert (setup[1] / "bodies" / first["page0_sha256"]).exists()


def test_hold_loss_between_queries_stops_before_second_probe(setup):
    def reply(request):
        if (
            request.url.params.get("url") == QUERIES[0]["original_url"]
            and request.url.params.get("page") == "0"
        ):
            (setup[0] / "operator-hold").unlink()
        return handler(request)

    with driver.accounting_connection(setup[0]) as conn:
        result = make(setup, conn, reply).run()
        assert conn.execute("SELECT SUM(requests) FROM host_budget").fetchone()[0] == 3
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM execution_admissions WHERE state!='settled'"
            ).fetchone()[0]
    ...
    assert result["status"] == "stopped_incomplete" and len(result["requests"]) == 3
    assert result["metadata_results"][0]["status"] == "captured_pending_independent_review"
    assert result["metadata_results"][1]["status"] == "pending"


def test_total_eight_mib_ceiling_can_stop_before_final_http_without_refunding_grant(setup):
    size = 2097151

    def reply(request):
        if request.url.path == "/robots.txt":
            return response(404, b"r" * size)
        prefix = b"1" if request.url.params.get("showNumPages") == "true" else b"[]"
        return response(body=prefix + b" " * (size - len(prefix)))

    with driver.accounting_connection(setup[0]) as conn:
        result = make(setup, conn, reply).run()
        assert tuple(
            conn.execute("SELECT SUM(requests),SUM(bytes) FROM host_budget").fetchone()
        ) == (5, 4 * size)
        assert (
            conn.execute(
                "SELECT COUNT(*) FROM execution_admissions WHERE state!='settled'"
    ...
    assert result["status"] == "stopped_incomplete" and len(result["requests"]) == 4
    assert result["received_bytes"] == 8388604 < MANIFEST["limits"]["max_total_received_bytes"]
    assert result["metadata_results"][1]["status"] == "probe_captured"
    assert "page0_sha256" not in result["metadata_results"][1]


def test_old_completed_operation_authority_cannot_authorize_new_scope(setup):
    setup[3]["owner_decision_reference"] = (
        "journal/decisions/0069-approve-exact-dcn-results-metadata-lookup.md"
    )
    with driver.accounting_connection(setup[0]) as conn:
        with pytest.raises(FixtureStopped, match="authorization"):
            make(setup, conn)
...
@pytest.mark.parametrize(
    "name", ["evidence/riga-results.html", "evidence/locator-context.json", "authority.md"]
)
def test_preparer_rejects_altered_locator_or_standing_authority_evidence(tmp_path, name):
    copied = tmp_path / "packet"
    shutil.copytree(PACKET, copied)
    (copied / "helper-closure" / name).write_text("changed")
    with pytest.raises(ValueError):
        preparation.verify_packet(copied)


@pytest.mark.parametrize("url", [q["original_url"] for q in QUERIES])
def test_printed_score_pdf_urls_are_never_request_authority(setup, url):
    with driver.accounting_connection(setup[0]) as conn:
        runner = make(setup, conn)
        with httpx.Client(
            transport=httpx.MockTransport(lambda request: pytest.fail("PDF body requested"))
        ) as client:
            with pytest.raises(FixtureStopped, match="allowlist"):
                runner._request(client, url, "cdx")


@pytest.mark.parametrize("floor", [25, 60])
def test_stricter_configured_floor_is_preserved_across_both_queries(setup, floor):
    with driver.accounting_connection(setup[0]) as conn:
        result = make(setup, conn, policy=HostConfig(min_gap_seconds=floor)).run()
    assert result["status"] == "captured_pending_independent_review"
...
def test_stricter_robots_crawl_delay_applies_to_every_metadata_request(setup):
    def reply(request):
        if request.url.path == "/robots.txt":
            return response(body=b"User-agent: swingset\nAllow: /\nCrawl-delay: 25\n")
        return handler(request)
...
def test_shared_daily_budget_can_stop_between_exact_queries(setup):
    state, _, clock, _ = setup
    with open_database(state) as db:
        db.connection.execute("INSERT INTO hosts(host) VALUES (?)", (HOST,))
        db.connection.execute(
            "INSERT INTO host_budget VALUES (?,?,197,17)", (HOST, clock.now().date().isoformat())
        )
        db.connection.execute(
            "INSERT INTO host_request_spacing VALUES (?,?,10,?,?)",
            (HOST, "reviewed_prior", clock.now().isoformat(), clock.now().isoformat()),
